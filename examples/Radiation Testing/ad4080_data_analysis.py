...
def get_adc_data(data_raw, sample_count):
...
def process_adc_raw_data(adc, adc_test_channel, data_raw, sample_count, fs, avg_value):
    ...
    print("Input data_dc (Counts):", data_dc, "\nInput data_dc (V):", data_dc * vlsb_diff)
    ...
    win = a0 - a1 * np.cos(2 * np.pi * t) + a2 * np.cos(4 * np.pi * t) - a3 * np.cos(6 * np.pi * t) + a4 * np.cos(
        8 * np.pi * t) - a5 * np.cos(10 * np.pi * t) + a6 * np.cos(12 * np.pi * t)

    win = win * norm

    # Compute FFT of filter
    win_fft = np.fft.fft(win, sample_count * 10) / sample_count  # To increase resolution to see lobes
    ...
    freq_domain_magnitude_db = 20 * np.log10((win_data_fft_mag[1:int(sample_count / 2) + 1]) / adc_amplitude_adj)
    ...
    try:
        harmonics, snr, thd, sinad, enob, sfdr, floor = sp.sin_params_ad4080(data_no_dc, num_harms=9)
        ...
        thd_calc = 10 * np.log10((10 ** (f2 / 10)) + (10 ** (f3 / 10)) + (10 ** (f4 / 10)) + (10 ** (f5 / 10)))
        sinad_calc = -10 * np.log10((10 ** (-snr_adj / 10)) + (10 ** (thd_calc / 10)))
        enob_calc = (sinad_calc - 1.76) / 6.02
        sfdr_adj = sfdr - fund_dbfs

        calculated_parameters = [("Sample Averaging", str(avg_value), "Samples"),
        ...
        print("Average Measured Voltage value: " + "{0:.6f}".format(round(np.mean(voltage_meas), 6)) + " V")
    ...
    ax_freq.plot(freq_array, freq_domain_magnitude_db, color='C0', zorder=2, label="Spectrum")
...
def validate_results(data_validation):
    ...
    for test_type, test_var, min_limit, max_limit in data_validation:
        ...
        print(f"\t{test_type : >8} = {round(test_var, 2) :<8}\tMin = {min_limit :<8}\tMax = {max_limit : <8}")
